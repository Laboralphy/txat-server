...
var TxatSystem = O2.createClass({
	aChannels: null,		// liste des canaux créés
	aUsers: null,			// liste des utilisateurs connecté
	oChannelRecycler: null,
	oUserRecycler: null,
	oMediator: null,
	oConfig: null,
	oChannelRegistry: null,
	oUserRegistry: null,
	...
	configure: function(oConfig) {
		// plugins
		this.initPlugins(['Log', 'Filter', 'Auth', 'Upref', 'Avatar']);
		this.sendSignal('config', oConfig.plugins);
		// canaux
	 ...
	 */
	initPlugins: function(aPlugins) {
		var m = new Mediator.Mediator();
		m.setApplication(this);
		aPlugins.forEach(function(i, n, a) {
			var P = require('./Plugins/' + i + 'Plugin.js');
			var p = new P();
			m.addPlugin(p);
		});
		this.oMediator = m;
	...
	checkUserLoginName: function(sLoginName) {
	 ...
	 */ 
	createUser: function(sLoginName) {
		var oUser = new User();
		var id = this.oUserRecycler.getId();
		oUser.id = id;
	 ...
	 */
	addUserToChannel: function(idUser,idChannel) {
	 ...
	 */
	removeUserFromChannel: function(idUser, idChannel) {
		var oChan = this.getChannel(idChannel);
		var oUser = this.getUser(idUser);
	 ...
	 */
	removeUserFromAllChannels: function(idUser) {
		var oUser = this.getUser(idUser);
	 ...
	 */
	banUserFromChannel: function(idBanned, idChannel, nTime, sWhy, idJudge) {
		if (nTime) {
	...
	unbanUserFromChannel: function(idBanned, idChannel, idJudge) {
		var oBanned = this.getUser(idBanned);
		var oJudge = this.getUser(idJudge);
		var oChan = this.getChannel(idChannel);
		var oPowBanned;
	...
	promoteUser: function(idChannel, idPadawan, idJedi, n) {
		if (n === undefined) {
			n = 1;
		}
	...
	demoteUser: function(idChannel, idPadawan, idJedi) {
	 ...
	 */
	getChannelUserList: function(idChannel) {
	 ...
	 * @return array of string
	 */
	getUserAccessibleChannels: function(idUser) {
	 ...
	 * @param object xData
	 */
	notifyChannelUsers: function(idChannel, sMessage, xData) {
	 ...
	 */
	userJoinsChannel: function(uid, sChannel) {
		var cid;
		try {
	 ...
	 */
	sendMessageToChannel: function(idUser, idChannel, sMessage) {
		var oUser = this.getUser(idUser);
		...
		for (var i = 0; i < nUserCount; ++i) {
			...
			if (oUserDst.oPowers[idChannel].bReceive) {
				this.doMessage(sMessage, oChannel, oUser, oUserDst);
			}
		}
	},
	
	sendMessageToUser: function(idUserSrc, idUserDst, sMessage) {
		...
		this.doMessage(sMessage, null, oUserSrc, oUserDst);
	},
	
	setUserPref: function(idUser, sVariable, sValue) {
		var oUser = this.getUser(idUser);
		if (sVariable.match(/^[A-Za-z][-_0-9A-Za-z]*$/)) {
			oUser.setData(sVariable, sValue);
			this.sendSignal('userPref', oUser, sVariable, sValue);
		} else {
			throw new Error('invalid user preference : ' + sVariable);
		}
	},
	 ...
	 */
	doMessage: function(sMessage, oFromChannel, oFromUser, oToUser) {
		if (this.onMessage) {
			this.onMessage(sMessage, oFromChannel, oFromUser, oToUser);
		}
	},
	...
	doNotify: function() {
		if (this.onNotify) {
			this.onNotify.apply(this, arguments);
		}
	}
});
