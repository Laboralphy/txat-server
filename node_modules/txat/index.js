	...
	oSocket.on(CONST.OPCODE.C_WHO, function(xData) {
		try {
			...
			if (idChannel.toString() in oUser.oPowers) {
				...
				var sInfo = util.format(CONST.MESSAGE.USER_ON_CHANNEL, p.getRankStr(), xData.c);
...
function txatMessage(sMessage, oFromChannel, oFromUser, oToUser) {
	if (oFromChannel) {
		aSocketRegistry[oToUser.id].emit(CONST.OPCODE.CM, {u: oFromUser.getDisplayName(), c: oFromChannel.sName, m: sMessage});
	} else {
		aSocketRegistry[oToUser.id].emit(CONST.OPCODE.UM, {u: oFromUser.getDisplayName(), m: sMessage});
	}
}
